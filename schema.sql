CREATE DATABASE IF NOT EXISTS LANTERN;
CREATE USER IF NOT EXISTS 'admin'@'localhost' IDENTIFIED BY 'strong_password';
GRANT ALL PRIVILEGES ON LANTERN.* TO 'admin'@'localhost';
FLUSH PRIVILEGES;
...
CREATE TABLE `users` (
  ...
  `auth_provider` varchar(50) COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT 'local',
...
CREATE TABLE `notes` (
  ...
  `updated_at` timestamp NULL DEFAULT NULL,
  PRIMARY KEY (`id`),
  CONSTRAINT `notes_task_id_foreign` FOREIGN KEY (`task_id`) REFERENCES `tasks` (`id`) ON DELETE CASCADE,
